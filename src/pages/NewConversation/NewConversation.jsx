...
const NewConversationPage = () => {
    ...
    const [modalOpen, setModalOpen] = useState({open: false, chatId: null, datetimeIndex: null});
    ...
    const handleModal = (isOpen, chatId, datetimeId) => {
        setModalOpen(prev => {
            return {...prev, open: isOpen, chatId: chatId, datetimeIndex: datetimeId}
        });
    }
    const updateFeedbackAndRatings = (key, value, chatIdIndex=null, datetimeIdIndex=null) => {
        if (key === 'feedback') {
    ...
    const handleChats = (value) => {
        ...
        const chatIdx = chats.findIndex(item => item['datetime'].getDate() === new Date().getDate());
    ...
    return (
        <Box>
            {
                chats.length <= 0 && <InitialChat handleChats={handleChats} />
            }
            {
                chats.length > 0 && 
                chats.map((item, idx) => {
                    ...
                    <Stack direction="column" spacing={2}>
                        <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between">
                            <Stack>
                                ...
                                }}>Provide Additional Feedback</p>

                            </Stack>
                            <IconButton onClick={() => setModalOpen(prev => {
