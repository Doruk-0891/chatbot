...
const PastConversationPage = () => {
    const [userAsked, isAsk, setAsk, chats, setChats, selectedRatings, setSelectedRatings] = useOutletContext();

    const [filteredChats, setFilteredChats] = useState(JSON.parse(localStorage.getItem('prevChats')) || []);
    ...
    const handleFilteredChats = (value) => {
        ...
        const filterdChatsArr = chatsArr.reduce((acc, item) => {
            ...
            const filteredChatsList = item['chatsList'].filter(singleChat => singleChat['aiResponse']['ratings'] === value);
    ...
    return (
        <Stack direction="column" spacing={5}>
            ...
            <Box display={"flex"} justifyContent={"center"}>
                <Dropdown value={selectedRatings} handleValue={(value) => handleFilteredChats(value)} />
            </Box>
                        ...
                        const {datetime, chatsList} = item;
                            return (
                                <Stack direction="column" spacing={2}>
                                    <Typography>
                                        {formatDate( new Date(datetime))} chats
                                    </Typography>
                                    <Stack direction="column" spacing={2}>
                                        {
                                            chatsList.map(item => {
